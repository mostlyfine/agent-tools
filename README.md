...
tmux セッション内で動作中の Claude Code / Copilot CLI / OpenAI Codex CLI プロセスを一覧表示するモニタリングツール。
...
- ウォッチモードでは行をマウスクリックすると、その pane へ選択+即フォーカス（要 SGR マウス対応ターミナル）
- `--horizontal`/`-H` でカード風の縦積みレイアウトに切り替え可能（狭いペイン向け）
...
# カード風の縦積みレイアウトで表示
scripts/agent-ps --horizontal
scripts/agent-ps -H

# ウォッチモード + カード風レイアウト
scripts/agent-ps -w -H
```
