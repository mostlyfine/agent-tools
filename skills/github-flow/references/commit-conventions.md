...
意味のある論理単位ごとに、Conventional Commits 形式（コミットメッセージは英語）でコミットする。作業ツリーがクリーンになるまで繰り返す。

## 目次

- [コミットワークフロー](#コミットワークフロー)
  - [Phase A: コミット範囲の決定](#phase-a-コミット範囲の決定)
  - [Phase B: 作業ツリーがクリーンになるまでループ](#phase-b-作業ツリーがクリーンになるまでループ)
...
### Phase B: 作業ツリーがクリーンになるまでループ

`git status` が未ステージングの変更も取り込むべき未追跡ファイルも示さなくなるまで、以下を繰り返す。
...
未追跡ファイルがあれば `AskUserQuestion` でどれをコミットに含めるかユーザーに確認する。明示的に確認が取れたファイルのみ含める。
...
変更を論理単位にグルーピングする。各単位は1つの一貫した変更（単一責任）を表すこと。提案するConventional Commitメッセージとともに計画を提示し、確認を待ってから進む。
