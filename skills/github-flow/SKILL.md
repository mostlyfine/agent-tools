...
コミットは「意味のある論理単位」ごとに分け、コミットすべき変更がなくなるまで繰り返す。
...
## Step 3: 論理単位ごとにコミット（コミットすべき変更がなくなるまでループ）

コミットに含めるべき変更が `git status` に残らなくなるまで以下を繰り返す。

1. **未追跡ファイル**の扱いを決める（初回のみ）
   ...
   - どちらとも判断できないもの → `AskUserQuestion` でコミットに含めるか確認する
2. `git diff HEAD` を分析し、変更を**論理単位（単一責任）**にグルーピングする。**Conventional Commits の type が異なる変更は別コミットに分ける**のが基準（例: 実装＋その検証テスト = `feat` の1コミット、README・ドキュメント追加 = `docs` の別コミット）。提案メッセージとともに計画を提示して確認を待つ
3. 次の単位をステージングする
   - ファイル単位: `git add <file1> <file2>` / 削除は `git rm <file>`
   - ハンク単位: `git apply --cached <patch-file>` でハンクを自動ステージング（ユーザー操作不要）
4. **Step 4** でコミット → コミットすべき変更が残っていれば 2 へ戻る

`git add .` / `git add -A` は非推奨。`.env`・認証情報・秘密鍵はステージングしない。
手順の詳細は `references/commit-conventions.md`（Phase B）を参照。

> **ユーザーに確認できない環境**（サブエージェントとしての実行・自動実行など）では、`AskUserQuestion` と確認待ちをスキップし、上記 1〜2 の基準で自律的に判断して進む。判断できなかった未追跡ファイルはコミットに含めない。スキップした確認と判断の理由は必ず最終報告に記録する。
